...
const MOODS = {
  night: {
    sky: ['#0d0e22', '#1f1d40', '#35305a'], wall: ['#25203a', '#1a1728'], city: '#110f1b',
    lights: 0.95, rain: 80, rainColor: C.rain, lamp: true, label: 'rainy night',
  },
  dusk: {
    sky: ['#3a2a5c', '#b96a78', '#f2a36f'], wall: ['#2c2339', '#1d1829'], city: '#291d33',
    lights: 0.6, rain: 45, rainColor: '#f3d9c8', lamp: true, label: 'rainy dusk',
  },
  morning: {
    sky: ['#86b3dc', '#c6def0', '#f6e7cf'], wall: ['#352f4c', '#28233a'], city: '#6a6f93',
    lights: 0, rain: 22, rainColor: '#ffffff', lamp: false, label: 'drizzly morning',
  },
  afternoon: {
    sky: ['#6b9fd1', '#a9cbe8', '#dcebf3'], wall: ['#332d49', '#262138'], city: '#5c618a',
    lights: 0, rain: 30, rainColor: '#ffffff', lamp: false, label: 'soft rain, afternoon',
  },
};
...
function sky(m, r, moodName) {
  const out = [];
  out.push(`<rect x="${GX}" y="${GY}" width="${GW}" height="${GH}" fill="url(#sky)"/>`);
  ...
  if (moodName === 'night') {
    ...
    out.push(`<circle cx="352" cy="118" r="46" fill="${C.cream}" opacity="0.08"/>
<circle cx="352" cy="118" r="24" fill="#f7efd9"/>
<circle cx="343" cy="112" r="4.5" fill="#e6dcc2"/><circle cx="359" cy="126" r="3" fill="#e6dcc2"/><circle cx="360" cy="108" r="2" fill="#e6dcc2"/>`);
  ...
  if (moodName === 'morning' || moodName === 'afternoon') {
    const sx = moodName === 'morning' ? 150 : 330, sy = moodName === 'morning' ? 150 : 110;
    out.push(`<circle cx="${sx}" cy="${sy}" r="44" fill="#fff6dc" opacity="0.35"/><circle cx="${sx}" cy="${sy}" r="26" fill="#fff3cf"/>`);
  }

  // drifting clouds
  const cloudFill = moodName === 'night' ? '#2c2a4d' : moodName === 'dusk' ? '#8a5a78' : '#ffffff';
  const cloudOp = moodName === 'night' ? 0.8 : 0.75;
  for (const [cx, cy, sc, dur] of [[120, 104, 1, 70], [300, 150, 0.8, 95], [220, 88, 0.65, 120]]) {
    out.push(`<g class="drift" style="animation-duration:${dur}s;animation-delay:-${(r() * dur).toFixed(1)}s" opacity="${cloudOp}">
  <g transform="translate(${cx} ${cy}) scale(${sc})" fill="${cloudFill}">
    <ellipse cx="0" cy="0" rx="44" ry="15"/><ellipse cx="-18" cy="-9" rx="20" ry="15"/><ellipse cx="14" cy="-13" rx="24" ry="18"/>
  </g></g>`);
  }
  ...
  const bldg = [];
  while (x < GX + GW) {
    const bw = 22 + r() * 34, bh = 40 + r() * 80;
    const by = GY + GH - bh;
    bldg.push(`<rect x="${x.toFixed(1)}" y="${by.toFixed(1)}" width="${bw.toFixed(1)}" height="${bh.toFixed(1)}" fill="${m.city}"/>`);
    if (m.lights > 0) {
      for (let wy = by + 8; wy < GY + GH - 8; wy += 12)
        for (let wx = x + 5; wx < x + bw - 6; wx += 9)
          if (r() < 0.32)
            bldg.push(`<rect class="${r() < 0.25 ? 'flick' : ''}" style="animation-delay:-${(r() * 9).toFixed(1)}s" x="${wx.toFixed(1)}" y="${wy.toFixed(1)}" width="4" height="5" rx="0.5" fill="${C.amber}" opacity="${(m.lights * (0.5 + r() * 0.5)).toFixed(2)}"/>`);
    }
    x += bw + 2;
  }
  out.push(bldg.join('\n'));
  ...
  // rain
  for (let i = 0; i < m.rain; i++) {
    const rx = GX + r() * (GW + 40), len = 10 + r() * 12;
    const dur = (0.55 + r() * 0.5).toFixed(2), delay = (r() * 2).toFixed(2);
    out.push(`<line class="drop" style="animation-duration:${dur}s;animation-delay:-${delay}s" x1="${rx.toFixed(1)}" y1="${GY - 30}" x2="${(rx - 4).toFixed(1)}" y2="${(GY - 30 + len).toFixed(1)}" stroke="${m.rainColor}" stroke-width="1.3" stroke-linecap="round" opacity="${(0.35 + r() * 0.45).toFixed(2)}"/>`);
  }
  // droplets sliding down the glass
  for (let i = 0; i < 7; i++) {
    const dx = GX + 12 + r() * (GW - 24);
    out.push(`<circle class="slide" style="animation-duration:${(7 + r() * 8).toFixed(1)}s;animation-delay:-${(r() * 12).toFixed(1)}s" cx="${dx.toFixed(1)}" cy="${GY + 10}" r="${(1.8 + r() * 1.6).toFixed(1)}" fill="#ffffff" opacity="0.35"/>`);
  }
  return out.join('\n');
}

function typing(lines) {
  const cw = 8.6, charDur = 0.07, pause = 0.9, hold = 3.2;
  const x0 = 596, y0 = 198, lh = 24;
  let t = 0.6;
  ...
  const out = [];
  const cur = { times: ['0'], x: [String(x0 + 16)], y: [String(y0 - 13)] };
  lines.forEach((line, i) => {
    const n = line.length, s = starts[i], y = y0 + i * lh;
    const kt = ['0'], vals = ['0'];
    for (let c = 0; c <= n; c++) {
      kt.push(k(s + c * charDur)); vals.push((c * cw).toFixed(1));
      cur.times.push(k(s + c * charDur)); cur.x.push((x0 + 16 + c * cw).toFixed(1)); cur.y.push(String(y - 13));
    }
    out.push(`<clipPath id="ln${i}"><rect x="${x0 + 16}" y="${y - 16}" height="22" width="0">
  <animate attributeName="width" dur="${T.toFixed(2)}s" repeatCount="indefinite" calcMode="discrete" keyTimes="${kt.join(';')}" values="${vals.join(';')}"/>
</rect></clipPath>
<text x="${x0}" y="${y}" font-size="14" fill="${C.sage}">›</text>
<text clip-path="url(#ln${i})" x="${x0 + 16}" y="${y}" font-size="14" fill="${i === 0 ? C.amber : C.cream}" textLength="${(n * cw).toFixed(1)}" lengthAdjust="spacingAndGlyphs">${esc(line)}</text>`);
  });
  out.push(`<rect class="blink" width="8" height="16" rx="1" fill="${C.amber}">
...
export function renderHeader({ config, moodName, clock }) {
  const m = MOODS[moodName];
  const r = rng(20251201);
  ...
  for (let i = 0; i < 26; i++) {
    const x = 30 + i * 44.5;
    const y = 22 + Math.sin((i / 25) * Math.PI * 3) * 6 + 10;
    fairy.push(`<line x1="${x}" y1="${y - 5}" x2="${x}" y2="${y}" stroke="#4a4266" stroke-width="1.5"/><circle class="fairy" style="animation-delay:-${(i * 0.37).toFixed(2)}s" cx="${x}" cy="${y + 4}" r="4.2" fill="${bulbs[i % bulbs.length]}"/><circle cx="${x}" cy="${y + 4}" r="11" fill="${bulbs[i % bulbs.length]}" opacity="0.18"/>`);
  }
  const wire = Array.from({ length: 27 }, (_, i) => `${(30 + i * 44.5).toFixed(1)},${(22 + Math.sin((i / 25) * Math.PI * 3) * 6 + 5).toFixed(1)}`).join(' ');

  const books = [
    { h: 16, w: 118, c: C.rose }, { h: 14, w: 104, c: C.sage }, { h: 18, w: 112, c: C.lilac }, { h: 13, w: 96, c: C.amber },
  ];
  let by = 332;
...
<defs>
  ...
  <linearGradient id="sky" x1="0" y1="0" x2="0" y2="1">
    ${m.sky.map((c, i) => `<stop offset="${i / (m.sky.length - 1)}" stop-color="${c}"/>`).join('')}
  </linearGradient>
  <linearGradient id="wall" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${m.wall[0]}"/><stop offset="1" stop-color="${m.wall[1]}"/></linearGradient>
...
<g clip-path="url(#card)">
  ...
  ${Array.from({ length: 11 }, (_, row) => Array.from({ length: 30 }, (_, col) => `<circle cx="${20 + col * 40 + (row % 2) * 20}" cy="${50 + row * 26}" r="1.2" fill="#ffffff" opacity="0.035"/>`).join('')).join('')}

  <!-- fairy lights -->
  <polyline points="${wire}" fill="none" stroke="#4a4266" stroke-width="1.5"/>
  ${fairy.join('\n  ')}

  <!-- window -->
  <rect x="${GX - 16}" y="${GY - 16}" width="${GW + 32}" height="${GH + 32}" rx="10" fill="${C.woodDark}"/>
  ...
  ${m.lamp ? '' : `<polygon points="${GX + GW},${GY + 20} ${GX + GW},${GY + GH} 900,420 560,420" fill="url(#beam)"/>
  ...
  <!-- chalkboard with live status -->
  <g transform="translate(490 44)">
    <rect x="0" y="0" width="250" height="86" rx="8" fill="${C.woodDark}"/>
    <rect x="6" y="6" width="238" height="74" rx="5" fill="#26302b"/>
    <text x="18" y="29" font-size="14" fill="${C.cream}" style="font-family:${HAND}" opacity="0.92">now · ${esc(clock)}</text>
    <text x="18" y="50" font-size="14" fill="${C.amber}" style="font-family:${HAND}">mood · ${esc(m.label)}</text>
    <text x="18" y="71" font-size="14" fill="${C.sage}" style="font-family:${HAND}">status · brewing code</text>
    <rect x="196" y="76" width="24" height="5" rx="2" fill="#e8e2d0" opacity="0.7"/>
  </g>
  ...
  <!-- lamp glow (on desk + wall) -->
  ${m.lamp ? `<ellipse class="glow" cx="930" cy="290" rx="380" ry="230" fill="url(#lamp)"/>` : ''}

  <!-- desk -->
  <rect x="0" y="330" width="${W}" height="${H - 330}" fill="url(#desk)"/>
  <rect x="0" y="330" width="${W}" height="3" fill="#8f6c56"/>
  ${[352, 372, 395].map((y, i) => `<path d="M0 ${y} Q 300 ${y + (i % 2 ? 6 : -5)} 600 ${y} T 1200 ${y}" stroke="#000" stroke-opacity="0.08" fill="none"/>`).join('')}

  <!-- manga stack -->
  ${bookSvg}
  <text x="130" y="${332 - 16 - 14 + 11}" font-size="9" fill="#2a2540" opacity="0.7">manga backlog</text>

  <!-- mug + steam -->
  <g transform="translate(470 0)">
    <path class="steam" style="animation-delay:0s" d="M18 262 q -8 -12 0 -24 q 8 -12 0 -24" stroke="${C.cream}" stroke-width="3" fill="none" stroke-linecap="round"/>
    <path class="steam" style="animation-delay:-1.2s" d="M32 266 q -8 -12 0 -24 q 8 -12 0 -24" stroke="${C.cream}" stroke-width="3" fill="none" stroke-linecap="round"/>
    <path class="steam" style="animation-delay:-2.4s" d="M46 262 q -8 -12 0 -24 q 8 -12 0 -24" stroke="${C.cream}" stroke-width="3" fill="none" stroke-linecap="round"/>
  ...
  <g transform="translate(596 304)">
    ${[0, 1, 2, 3, 4].map((i) => `<rect class="eq" style="animation-delay:-${(i * 0.23).toFixed(2)}s" x="${i * 6}" y="-12" width="3.5" height="12" rx="1.5" fill="${C.lilac}"/>`).join('')}
    <text x="40" y="-2" font-size="11" fill="${C.muted}">lofi beats to code/relax to</text>
  </g>
  ...
  <g>
    ...
    <g transform="translate(1004 184) rotate(38)">
      ${m.lamp ? `<polygon points="-26,4 26,4 100,150 -100,150" fill="url(#cone)"/>` : ''}
      ...
      ${m.lamp ? `<ellipse class="glow" cx="0" cy="6" rx="15" ry="6" fill="#fff6dc"/>` : `<ellipse cx="0" cy="6" rx="13" ry="5" fill="#8f8aa6"/>`}
    </g>
  </g>

  <!-- little radio with floating notes -->
...
`;
  ...
  return svg(W, H, body, { title: `a cozy ${m.label} desk scene: hi, i'm ${config.name} (${config.alias}), ${config.role}`, style });
}
